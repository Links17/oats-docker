...
import (
	...
	"net"
	"net/http"
	"oats-docker/pkg/container/meta"
	"oats-docker/pkg/container/registry/auth"
	"oats-docker/pkg/container/registry/manifest"
...
// CompareDigest ...
func CompareDigest(container types.Container, registryAuth string, tag string) (bool, error) {
	...
	token, err := auth.GetToken(container, registryAuth, tag)
	if err != nil {
		return false, err
	}

	digestURL, err := manifest.BuildManifestURL(container, tag)
	if err != nil {
		return false, err
	}
