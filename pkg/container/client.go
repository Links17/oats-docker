...
import (
	...
	"io"
	"oats-docker/pkg/container/registry"
	"oats-docker/pkg/container/registry/digest"
...
type Client interface {
	...
	ImageID(tag string) (latestImage string, err error)
	CreateContainer(config *container.Config, networkingConfig *network.NetworkingConfig, hostConfig *container.HostConfig) (t.ContainerID, error)
	PullImageByTag(ctx context.Context, tag string) error
...
func (client dockerClient) ListContainers() ([]t.Container, error) {
	cs := []t.Container{}
...
func (client dockerClient) GetContainerByName(name string) (*types.Container, error) {
	filter := filters.NewArgs()
	filter.Add("name", name)
	fmt.Println(name)
	containers, err := client.api.ContainerList(context.Background(), types.ContainerListOptions{Filters: filter, All: true})
	if err != nil {
...
func (client dockerClient) CreateContainer(config *container.Config, networkingConfig *network.NetworkingConfig, hostConfig *container.HostConfig) (t.ContainerID, error) {
	bg := context.Background()
	// 检查container Name 是否存在,如果存在则不继续
	_, err := client.GetContainerByName(config.Hostname)
	if err == nil {
		return "", fmt.Errorf("container exist")
	}
	check := util.CheckContainerPort(hostConfig)
	if !check {
		return "", fmt.Errorf("port is already allocated")
	}
	// 检查port是否被占用,如果被占用则不继续

	err = client.PullImageByTag(bg, config.Image)
	if err != nil {
		return "", err
	}
	...
	return createdContainerID, client.doStartContainerByName(bg, name, createdContainer)
...
func (client dockerClient) doStartContainerByName(bg context.Context, name string, creation container.CreateResponse) error {
	log.Debugf("Starting container %s (%s)", name, t.ContainerID(creation.ID).ShortID())
	err := client.api.ContainerStart(bg, creation.ID, types.ContainerStartOptions{})
	if err != nil {
		return err
...
func (client dockerClient) IsContainerStale(container t.Container, params t.UpdateParams) (stale bool, latestImage t.ImageID, err error) {
	...
	tag := util.RepoTag(container.Name(), params)
	if tag == "" {
		return false, "", fmt.Errorf("tag is empty,doesn't update")
	}
	if container.IsNoPull(params) {
		log.Debugf("Skipping image pull.")
	} else if err := client.PullImage(ctx, container, tag); err != nil {
		return false, container.SafeImageID(), err
	}
	return client.HasNewImage(ctx, container, tag)
}

func (client dockerClient) HasNewImage(ctx context.Context, container t.Container, tag string) (hasNew bool, latestImage t.ImageID, err error) {
...
// Find ImageID
func (client dockerClient) ImageID(tag string) (latestImage string, err error) {
	ctx := context.Background()
	newImageInfo, _, err := client.api.ImageInspectWithRaw(ctx, tag)
	if err != nil {
		return "", err
	}
	return newImageInfo.ID, nil
}
...
func (client dockerClient) PullImage(ctx context.Context, container t.Container, tag string) error {
	...
	if match, err := digest.CompareDigest(container, opts.RegistryAuth, tag); err != nil {
...
func (client dockerClient) PullImageByTag(ctx context.Context, tag string) error {
	opts, err := registry.GetPullOptions(tag)
	if err != nil {
		log.Debugf("Error loading authentication credentials %s", err)
		return err
	}
	...
	response, err := client.api.ImagePull(ctx, tag, opts)
	if err != nil {
		log.Debugf("Error pulling image %s, %s", tag, err)
		return err
	}
